...
HerderImpl::sendSCPStateToPeer(uint32 ledgerSeq, PeerPtr peer)
{
    ...
    assert(mSCP.getLowSlotIndex() <= std::numeric_limits<uint32_t>::max());
    assert(mSCP.getHighSlotIndex() <= std::numeric_limits<uint32_t>::max());

    auto minSeq =
        std::max(ledgerSeq, static_cast<uint32_t>(mSCP.getLowSlotIndex()));
    auto maxSeq = static_cast<uint32_t>(mSCP.getHighSlotIndex());
